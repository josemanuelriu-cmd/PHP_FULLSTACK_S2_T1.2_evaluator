...
SELECT nombre as `nom del producte`, precio as euros, precio *1.1 as dòlars FROM producto;
...
SELECT p.nombre, p.precio, f.nombre as 'nombre del fabricante' FROM producto p JOIN fabricante f ON f.codigo=p.codigo_fabricante;
...
SELECT p.nombre, p.precio, f.nombre as 'fabricante' FROM producto p JOIN fabricante f ON f.codigo=p.codigo_fabricante WHERE f.nombre IN ('Asus','Hewlett-Packard','Seagate');
...
SELECT p.nombre, p.precio, f.nombre as 'fabricante' FROM producto p JOIN fabricante f ON f.codigo=p.codigo_fabricante WHERE p.precio>=180 ORDER BY p.precio DESC, p.nombre ASC;
...
SELECT p.* FROM producto p, fabricante f WHERE f.codigo=p.codigo_fabricante AND f.nombre='Lenovo';

-- 37. Retorna totes les dades dels productes que tenen el mateix preu que el producte més car del fabricant Lenovo. (Sense usar INNER JOIN).
SELECT * FROM producto WHERE precio = (SELECT MAX(p.precio) FROM producto p JOIN fabricante f ON f.codigo=p.codigo_fabricante WHERE f.nombre='Lenovo');
...
SELECT p.codigo, p.nombre, p.precio, f.codigo AS codigo_fabricante FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE f.nombre = 'Asus' AND p.precio > 
  ( SELECT AVG(p.precio) FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE f.nombre = 'Asus');
